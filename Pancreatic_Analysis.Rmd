...
output_dir <- "output/Pancreatic_Output"
...
# Load and preprocess data for Pancreatic Cancer
pancreatic_df = load_and_convert_data("data/Organized_Bruckner_Data.csv", "Pancreatic Cancer")
...
#resistant_pancreatic_df = subset_by_resistant(pancreatic_df, "Resistant")
...
def subset_by_cancer_status(df, cancer_status):
    ...
    Parameters:
        df (pd.DataFrame): The DataFrame to subset.
        cancer_status (str): The value to subset by (e.g., 'NPT- Pancreatic Cancer').
...
# Subset data frames for NPT- Pancreatic Cancer and Resistant- Pancreatic Cancer
npt_pancreatic_df = subset_by_cancer_status(pancreatic_df, 'NPT- Pancreatic Cancer')
resistant_pancreatic_df = subset_by_cancer_status(pancreatic_df, 'Resistant- Pancreatic Cancer')
...
resistant_pancreatic_df['Risk_Group_ALAN'] = pd.cut(resistant_pancreatic_df['Prognostic_Score_ALAN'], bins=bins, labels=labels, include_lowest=False)
npt_pancreatic_df['Risk_Group_ALAN'] = pd.cut(npt_pancreatic_df['Prognostic_Score_ALAN'], bins=bins, labels=labels, include_lowest=False)
...
print("\nSubsetted NPT Pancreatic Cancer Data Frame:")
print(npt_pancreatic_df.head())
print("\nSubsetted Resistant Pancreatic Cancer Data Frame:")
print(resistant_pancreatic_df.head())
...
print(npt_pancreatic_df.dtypes)
print(resistant_pancreatic_df.dtypes)

```
## Numeric Summary

Step 1: calculate the numeric statistics of the pancreatic_df
...
# Create summary statistics table for NPT pancreatic cancer
npt_pancreatic_stats = calculate_numeric_statistics(npt_pancreatic_df)

# Create summary statistics table for resistant pancreatic cancer
resistant_pancreatic_stats = calculate_numeric_statistics(resistant_pancreatic_df)

# Display the tables
print("Summary statistics for NPT Pancreatic cancer:")
print(npt_pancreatic_stats)
print("\nSummary statistics for resistant pancreatic cancer:")
print(resistant_pancreatic_stats)
```
...
# Generate the first table for Resistant Pancreatic Cancer
resistant_pancreatic_table <- calc_num_stats(py$resistant_pancreatic_df, selected_labels = c("Quantiles", "Percentiles"), percentiles = c(33, 67), title = "Numeric Statistics for Resistant Pancreatic Cancer")

# Save the table as an image
gtsave(resistant_pancreatic_table, filename = file.path(output_dir, "resistant_pancreatic_table.png"))

# Generate the second table for NPT Pancreatic Cancer
npt_pancreatic_table <- calc_num_stats(py$npt_pancreatic_df, selected_labels = c("Quantiles", "Percentiles"), percentiles = c(33, 67), title = "Numeric Statistics for NPT Pancreatic Cancer")

# Save the table as an image
gtsave(npt_pancreatic_table, filename = file.path(output_dir, "npt_pancreatic_table.png"))
...
def plot_numeric_statistics(df, variable, subset):
    ...
    plt.savefig(f'output/Pancreatic_Output/{subset}_{variable}_plot.png')
...
for column in npt_pancreatic_df.select_dtypes(include=['int64', 'float64']).columns:
    if column not in exclude_columns:
        plot_numeric_statistics(npt_pancreatic_df, column, 'NPT_Pancreatic_Cancer')
        
# Iterate over each numeric variable in your dataset and call the plot_numeric_statistics function
for column in resistant_pancreatic_df.select_dtypes(include=['int64', 'float64']).columns:
    if column not in exclude_columns:
        plot_numeric_statistics(resistant_pancreatic_df, column, 'Resistant_Pancreatic_Cancer')
...
categorized_npt_pancreatic_df <- categorize_values(py$npt_pancreatic_df)
categorized_resistant_pancreatic_df <- categorize_values(py$resistant_pancreatic_df)

# Check the result
print("Categorized NPT Pancreatic DataFrame:")
print(head(categorized_npt_pancreatic_df))

print("\nCategorized Resistant Pancreatic DataFrame:")
print(head(categorized_resistant_pancreatic_df))
...
# Convert columns to factors for categorized_npt_pancreatic_df
categorized_npt_pancreatic_df <- convert_to_factors(categorized_npt_pancreatic_df, columns_to_convert)

# Convert columns to factors for categorized_resistant_pancreatic_df
categorized_resistant_pancreatic_df <- convert_to_factors(categorized_resistant_pancreatic_df, columns_to_convert)

# Check the structure of the dataframes
str(categorized_npt_pancreatic_df)
str(categorized_resistant_pancreatic_df)
...
Calculate the Categorical statistics for our new pancreatic data frame
...
categorized_npt_pancreatic_stats = calculate_categorical_statistics(r.categorized_npt_pancreatic_df)
categorized_resistant_pancreatic_stats = calculate_categorical_statistics(r.categorized_resistant_pancreatic_df)

print(categorized_npt_pancreatic_stats)
print(categorized_resistant_pancreatic_stats)
...
save_gt_as_image <- function(table, filename) {
  gtsave(table, filename = filename, path = "output/Pancreatic_Output")
}

# Call the calc_cat_stats function and save the resulting gt tables
cat_stats_npt_pancreatic <- calc_cat_stats(categorized_npt_pancreatic_df, title = "Categoric Statistics for NPT- Pancreatic Cancer")
save_gt_as_image(cat_stats_npt_pancreatic, "categoric_stats_npt_pancreatic.png")

cat_stats_resistant_pancreatic <- calc_cat_stats(categorized_resistant_pancreatic_df, title = "Categoric Statistics for Resistant- Pancreatic Cancer")
save_gt_as_image(cat_stats_resistant_pancreatic, "categoric_stats_resistant_pancreatic.png")
...
plot_combined_categorical_statistics(categorized_npt_pancreatic_stats, title="Categorical Statistics for NPT- Pancreatic Cancer")
plot_combined_categorical_statistics(categorized_resistant_pancreatic_stats, title="Categorical Statistics for Resistant- Pancreatic Cancer")
...
# Create survival object for categorized_resistant_pancreatic_df
surv_obj_resistant <- Surv(time = categorized_resistant_pancreatic_df$time_diff_months, event = categorized_resistant_pancreatic_df$Event_Status)

# Create survival object for categorized_npt_pancreatic_df
surv_obj_npt <- Surv(time = categorized_npt_pancreatic_df$time_diff_months, event = categorized_npt_pancreatic_df$Event_Status)
```
...
resistant_fit <- survfit(Surv(time = categorized_resistant_pancreatic_df$time_diff_months, event = categorized_resistant_pancreatic_df$Event_Status) ~1, data = categorized_resistant_pancreatic_df)

resistant_kmplot <- ggsurvplot(resistant_fit,
                              data = categorized_resistant_pancreatic_df,
                              title = "Survival Curve for Resistant Pancreatic Cancer",
                              censor = TRUE,
                              xlab = "Time (Months)",
                              ...
                              surv.scale = "percent",
                              legend.labs = paste("Resistant- Pancreatic Cancer (N =", nrow(categorized_resistant_pancreatic_df),")"),
                              palette = "lancet")
...
npt_fit <- survfit(Surv(time = categorized_npt_pancreatic_df$time_diff_months, event = categorized_npt_pancreatic_df$Event_Status) ~1, data = categorized_npt_pancreatic_df)

npt_kmplot <- ggsurvplot(npt_fit,
                              data = categorized_npt_pancreatic_df,
                              title = "Survival Curve for No Prior Treatment Pancreatic Cancer",
                              censor = TRUE,
                              xlab = "Time (Months)",
                              ylab = "Survival Probability",
                              conf.int = TRUE,
                              ...
                              breaks = seq(0, 24, by = 3),
                              surv.scale = "percent",
                              legend.labs = paste("NPT- Pancreatic Cancer (N =", nrow(categorized_npt_pancreatic_df),")"),
                              palette = "lancet")
...
ggsave(filename = "output/Pancreatic_Output/resistant_kmplot.png", plot = resistant_kmplot_gg, width = 10, height = 6)
ggsave(filename = "output/Pancreatic_Output/npt_kmplot.png", plot = npt_kmplot_gg, width = 10, height = 6)
...
combined_kmplot <- ggsurvplot_combine(combined_fit,
                                  data = data_resistant,
                                  title = "Survival Curve for Resistant- vs. NPT- Pancreatic Cancer",
                                  censor = TRUE,
                                  xlab = "Time (Months)",
                                  ylab = "Survival Probability",
                                  conf.int = TRUE,
...
# Save the ggplot objects
ggsave(filename = "output/Pancreatic_Output/resistant_v_npt_kmplot.png", plot = combined_gg, width = 10, height = 6)
```


```{r rename df}
npt_df <- categorized_npt_pancreatic_df
resistant_df <- categorized_resistant_pancreatic_df
...
# Initialize a list to store survival fits for resistant Pancreatic Cancer 
resistant_km_fits <- list()

# Loop through variables for resistant Pancreatic Cancer
...
print("Resistant Pancreatic Cancer Survival Fits:")
...
# Initialize a list to store survival fits for NPT Pancreatic Cancer 
npt_km_fits <- list()

# Loop through variables for NPT Pancreatic Cancer
for (col in column_names) {
  # Construct formula with variable name extracted from column name
...
# Access results using names like resistant_km_fit_Albumin_3.5 etc.
print("NPT- Pancreatic Cancer Survival Fits:")
print(npt_km_fits)
...
These results suggest that factors like Albumin, ANC, NLR, and Alk_Phos levels might be associated with survival outcomes in patients with resistant pancreatic cancer, while other factors like LMR, MON, LY, PLT, and age may not have significant associations.
...
# Initialize an empty data frame to store log-rank test results for Resistant Pancreatic Cancer
...
for (col in column_names) {
  if (grepl("^Prognostic_Score_ALAN_Category", col)) {
    ...
    # Print log-rank test information for Resistant Pancreatic Cancer
    cat(rep("-", 20), "\n")
    cat("Log-rank tests for Resistant Pancreatic Cancer -", col, "\n")
    cat(rep("-", 20), "\n")
...
kable(log_rank_results_df_resistant, caption = "Log-rank Test Results for Resistant Pancreatic Cancer")
...
kable(log_rank_results_df_npt, caption = "Log-rank Test Results for npt- pancreatic cancer")
...
In summary, these results indicate that the prognostic score categories have significant or marginally significant differences in survival among the patients with resistant pancreatic cancer.
...
# Print the results
print("Pairwise log-rank test results for resistant pancreatic cancer:")
...
# Print the results
print("Pairwise log-rank test results for npt- pancreatic cancer:")
...
# Initialize lists to store Cox models, p-values, and hazard ratios for Resistant Pancreatic Cancer
...
for (col in column_names) {
  ...
  formula_resistant <- as.formula(paste("Surv(time_diff_months, Event_Status) ~", col))
    ...
    # Print Cox model for Resistant Pancreatic Cancer
    cat(rep("-", 30), "\n")
    cat("Cox Proportional Hazards for Resistant Pancreatic Cancer  -", col, "\n")
    cat(rep("-", 30), "\n")
    ...
    if (variable == "Prognostic_Score_ALAN_category") {
      ...
      # Print p-value for Resistant Pancreatic Cancer
    ...
    # Print Hazard Ratio for Resistant Pancreatic Cancer
    cat("Hazard Ratio:", cox_hazard_ratio_resistant, "\n")
    
    # Append the results to respective lists for Resistant Pancreatic Cancer
...
# Convert lists to data frames for Resistant Pancreatic Cancer
...
# Merge the two data frames for Resistant Pancreatic Cancer
...
# Print the combined data frame for Resistant Pancreatic Cancer using kable
kable(coxph_df_resistant, caption = "Cox Proportional Hazards Results for Resistant Pancreatic Cancer")

# Print the structure of the combined data frame for Resistant Pancreatic Cancer
...
kable(coxph_df_npt, caption = "Cox Proportional Hazards Results for NPT- Pancreatic Cancer")
...
# Initialize lists to store Schoenfeld test results and plots for Resistant Pancreatic Cancer
...
for (col in column_names) {
  ...
  # Fit Cox model for Resistant Pancreatic Cancer
  cox_model_resistant <- coxph(formula_resistant, data = resistant_df)
  
  # Perform Schoenfeld test for Resistant Pancreatic Cancer
  schoenfeld_test_resistant <- cox.zph(cox_model_resistant)
  
  # Print Schoenfeld test results for Resistant Pancreatic Cancer
  cat(rep("-", 45), "\n")
  cat("Schoenfeld Test for Resistant Pancreatic Cancer -", col, "\n")
  cat(rep("-", 45), "\n")
  print(schoenfeld_test_resistant)
  
  # Store Schoenfeld test result for Resistant Pancreatic Cancer in the list
  schoenfeld_results_list_resistant[[paste("schoenfeld_test", tolower(col), sep = "_")]] <- schoenfeld_test_resistant
  
  # Plot Schoenfeld residuals using ggcoxzph for Resistant Pancreatic Cancer
  schoenfeld_plot_resistant <- ggcoxzph(schoenfeld_test_resistant, caption = paste("Schoenfeld Plot of Resistant Pancreatic Cancer for residuals of", col))
  
  # Store Schoenfeld plot for Resistant Pancreatic Cancer in the list
  schoenfeld_plots_list_resistant[[paste("schoenfeld_plot", tolower(col), sep = "_")]] <- schoenfeld_plot_resistant
  
  # Print the plot for Resistant Pancreatic Cancer
  print(schoenfeld_plot_resistant)
}

# Access results using names like schoenfeld_test_ly etc. for Resistant Pancreatic Cancer
...
# Initialize lists to store Schoenfeld test results and plots for No Prior Treatment Pancreatic Cancer (NPT)
...
for (col in column_names) {
  ...
  cat(rep("-", 45), "\n")
  cat("Schoenfeld Test for NPT- Pancreatic Cancer", col, "\n")
  cat(rep("-", 45), "\n")
  ...
  schoenfeld_plot_npt <- ggcoxzph(schoenfeld_test_npt, caption = paste("Schoenfeld Plot of No Prior Treatment Pancreatic Cancer for residuals of", col))
...
cat(rep("-", 40), "\n")
print("Log-Rank pvalue and HR for NPT Pancreatic Cancer")
cat(rep("-", 40), "\n")
...
# Print the Log-Rank P-value and Hazard Ratio for each variable
cat(rep("-", 40), "\n")
print("Log-Rank pvalue and HR for Resistant Pancreatic Cancer")
cat(rep("-", 40), "\n")
...
```{r NPT KM Plot }
# Open a PDF device
pdf("output/Pancreatic_Output/npt_km_plots.pdf")
...
for (variable in variables_of_interest) {
  ...
  km_plot <- ggsurvplot(
    km_fit,
    data = npt_df,
    title = paste("Kaplan-Meier Curve of NPT- Pancreatic Cancer by", variable),
    censor = TRUE,
    xlab = "Time (Months)",
    ylab = "Survival Probability",
    conf.int = TRUE,
...
# Open a PDF device
pdf("output/Pancreatic_Output/resistant_km_plots.pdf")
...
for (variable_resistant in variables_of_interest_resistant) {
  ...
  km_plot_resistant <- ggsurvplot(
    km_fit_resistant,
    data = resistant_df,
    title = paste("Kaplan-Meier Curve of Resistant Pancreatic Cancer by", variable_resistant),
    censor = TRUE,
    xlab = "Time (Months)",
    ylab = "Survival Probability",
    conf.int = TRUE,
...
km_plot <- ggsurvplot(
  km_fit,
  data = npt_df,
  title = paste("Kaplan-Meier Curve of NPT- Pancreatic Cancer by ALAN Score"),
  censor = TRUE,
  xlab = "Time (Months)",
  ylab = "Survival Probability",
  conf.int = TRUE,
...
ggsave(filename = "output/Pancreatic_Output/KM Plots/npt_pancreatic_ALAN_kmplot.png", plot = km_plot, width = 10, height = 6)
...
km_plot_resistant <- ggsurvplot(
  km_fit_resistant,
  data = resistant_df,
  title = paste("Kaplan-Meier Curve of Resistant Pancreatic Cancer by ALAN Score"),
  censor = TRUE,
  xlab = "Time (Months)",
  ylab = "Survival Probability",
  conf.int = TRUE,
...
ggsave(filename = "output/Pancreatic_Output/KM Plots/resistant_pancreatic_ALAN_kmplot.png", plot = km_plot_resistant, width = 10, height = 6)
